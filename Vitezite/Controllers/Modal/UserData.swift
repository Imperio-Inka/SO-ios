...
class UserData:NSObject{
    ...
    var schollName:String  {
        set {
            NSUserDefaults.standardUserDefaults().setValue(newValue, forKeyPath: SCHOLLNAME)
        }
        get { if let label =  NSUserDefaults.standardUserDefaults().valueForKey(SCHOLLNAME)
        {
    ...
    var phoneNo:String  {
        set {
            NSUserDefaults.standardUserDefaults().setValue(newValue, forKeyPath: PHONENOKEY)
        }
        get { if let label =  NSUserDefaults.standardUserDefaults().valueForKey(PHONENOKEY)
        {
            return (label as? String)!
            }
    ...
    func getCurrentUserData()-> UserData{
        ...
        if (userDefault.boolForKey(ISREPEATED)) {
            ...
            user._id = userDefault.valueForKey(USERID) as! String;
            
            // user.schollName = userDefault.valueForKey(SCHOLLNAME) as! String
        }
    ...
    func setCurrentUser(user:UserData){
        ...
        userDefault.setValue(user.email, forKey: USEREMAIL)
        userDefault.setValue(user._id, forKey:USERID)
    ...
    func removeCurrentUser() {
        ...
        userDefault.removeObjectForKey(SCHOLLNAME)
    }
